"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.AddressRepository = void 0;
const tslib_1 = require("tslib");
const repositories_1 = require("../../repositories");
const repository_1 = require("@loopback/repository");
const models_1 = require("../../models");
let AddressRepository = class AddressRepository extends repositories_1.BaseLocalRepository {
    constructor(dataSource, addressTypeRepositoryGetter, stateRepositoryGetter) {
        super(models_1.Address, dataSource);
        this.addressType = this.createBelongsToAccessorFor('addressType', addressTypeRepositoryGetter);
        this.state = this.createBelongsToAccessorFor('state', stateRepositoryGetter);
        this.registerInclusionResolver('addressType', this.addressType.inclusionResolver);
        this.registerInclusionResolver('state', this.state.inclusionResolver);
    }
};
AddressRepository = (0, tslib_1.__decorate)([
    (0, tslib_1.__param)(1, repository_1.repository.getter('AddressTypeRepository')),
    (0, tslib_1.__param)(2, repository_1.repository.getter('StateRepository')),
    (0, tslib_1.__metadata)("design:paramtypes", [repository_1.juggler.DataSource, Function, Function])
], AddressRepository);
exports.AddressRepository = AddressRepository;
//# sourceMappingURL=address.repository.js.map