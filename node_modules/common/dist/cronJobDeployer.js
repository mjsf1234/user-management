...
async function createJob(config) {
    ...
    client
        .getJob({ name: jobName })
        .then(async (data) => {
        utils_1.LoggingUtils.info(`Job Exist ?  ${data[0].name}`);
        const [response] = await client.deleteJob({ name: data[0].name });
        utils_1.LoggingUtils.info(`Existed job deleted ?: ${JSON.stringify(response)}`);
    })
        .catch(error => {
        utils_1.LoggingUtils.error(`Job not found ? , ${error.message}`);
    })
        .finally(async () => {
        const [response] = await client.createJob(request);
        utils_1.LoggingUtils.info(`Created job: ${response.name}`);
    });
}
exports.configuration.forEach(config => {
    createJob(config).catch(err => {
        utils_1.LoggingUtils.error(err);
        process.exitCode = 1;
    });
});
//# sourceMappingURL=cronJobDeployer.js.map