...
class RoleResolver {
    constructor(requestObj, userProfile) {
        //call this function to get the resolved roles
        this.resolveRoles = () => {
            const roles = [];
            if (this.isOwner())
                roles.push('$owner');
            if (this.isPrimaryAccountOwner())
                roles.push('$primaryAccount');
            return roles;
        };
        this.isOwner = () => {
            if (this.modelName == 'AppUser' ||
                ...
                if (this.userProfile &&
                    ...
                    if (!this.requestQuery.contextFilter.accountIds ||
                    ...
                    else if (!utils_1.CompareUtils.isASupersetOfB(this.userProfile.accessibleAccountIds, this.requestQuery.contextFilter.accountIds)) {
                        return false;
                    }
                    if (!this.requestQuery.contextFilter.appUserIds ||
                    ...
                    else if (!utils_1.CompareUtils.isASupersetOfB(this.userProfile.accessibleAppUserIds, this.requestQuery.contextFilter.appUserIds)) {
                        return false;
                    }
                    if (!this.requestQuery.contextFilter.familyIds ||
                    ...
                    else if (!utils_1.CompareUtils.isASupersetOfB(this.userProfile.accessibleFamilyIds, this.requestQuery.contextFilter.familyIds)) {
                        return false;
                    }
                    return true;
                }
