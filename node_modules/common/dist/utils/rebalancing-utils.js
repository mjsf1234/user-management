...
exports.RebalancingUtils = void 0;
const constants_1 = require("../constants");
const logging_utils_1 = require("./logging-utils");
const goal_utils_1 = require("./goal-utils");
class RebalancingUtils {
    static doGoalRebalancing(goalInformation, portfolioData, modelPortfolio, isLumpsum) {
        ...
        let categoriesNames = constants_1.Option.GLOBALOPTIONS.CATEGORIES.name;
        ...
        let categoryGroupAssetExposure = constants_1.Option.GLOBALOPTIONS.CATEGORYGROUPASSETEXPOSURE.data;
        //this could also be master Data but calculating for now
        //let categoryCrossExposure = {};
        let categoryCategoryGroupExposure = {};
        //****** MAIN WORK STARTS ******//
        //SYSTEM VARIABLES
        let precisionPoint = constants_1.Option.GLOBALOPTIONS.REBALANCINGSETTINGS.precisionPoint; //rebalancing till 0.001%
        let percentCentBase = constants_1.Option.GLOBALOPTIONS.REBALANCINGSETTINGS.percentCentBase; //1 => 100%, 5.23% shall be like 0.0523 in data (change to 100 otherwise)
        let cleanUpNotRecommendedSchemes = constants_1.Option.GLOBALOPTIONS.REBALANCINGSETTINGS.cleanUpNotRecommendedSchemes;
        //Tolerance for regular alerts
        let goalShortfallTolerance = constants_1.Option.GLOBALOPTIONS.REBALANCINGSETTINGS.goalShortfallTolerance; //10%
        let assetClassDeviationTolerance = constants_1.Option.GLOBALOPTIONS.REBALANCINGSETTINGS.assetClassDeviationTolerance; //5%
        let categoryGroupDeviationTolerance = constants_1.Option.GLOBALOPTIONS.REBALANCINGSETTINGS.categoryGroupDeviationTolerance; //5%
        //Tolerance for urgent alerts
        let goalShortfallToleranceUrgent = constants_1.Option.GLOBALOPTIONS.REBALANCINGSETTINGS.goalShortfallToleranceUrgent; //20%
        let assetClassDeviationToleranceUrgent = constants_1.Option.GLOBALOPTIONS.REBALANCINGSETTINGS.assetClassDeviationToleranceUrgent; //10%
        let categoryGroupDeviationToleranceUrgent = constants_1.Option.GLOBALOPTIONS.REBALANCINGSETTINGS.categoryGroupDeviationToleranceUrgent; //10%
        //Tolerance in waterfall approach for buys
        let waterfallTolerance = constants_1.Option.GLOBALOPTIONS.REBALANCINGSETTINGS.waterfallTolerance;
        let maxSchemeWeight = constants_1.Option.GLOBALOPTIONS.REBALANCINGSETTINGS.maxSchemeWeight;
        //INPUT DATA VARIABLES
        let additionalBuy = 0;
        //CALCULATION VARIABLES
        let assetClassTotal = [];
        //let categoriesTotal = [];
        let categoryGroupTotal = [];
        let categoryGroupDeviationsTotal = [];
        let assetClassDeviationsTotal = [];
        ...
        //buyPriorityArray
        let buyPriorityArray = constants_1.StaticData.REBALANCINGDATA.BUYPRIORITY.data;
        ...
        function mainRebalancingHandler() {
            categoryCategoryGroupExposure = goal_utils_1.GoalUtils.createCategoryCrossExpMatrix();
            logging_utils_1.LoggingUtils.info('done categoryCategoryGroupExposure');
            initialize();
            logging_utils_1.LoggingUtils.info('done initialization');
            ...
            if (inputObject.fullRebalance) {
                if (toDoRebalancing) {
                    ...
                    if (inputObject.additionalLumpsumBuy > 0) {
                        ...
                        logging_utils_1.LoggingUtils.info('Rebalancing not needed as there is no deviation. We will process the additional buy needed.');
                        returnObject.processSuccessful = true;
                        returnObject.rebalancingProcessSuccess = false;
                    }
                    else {
                        logging_utils_1.LoggingUtils.info('Rebalancing not needed as there is no deviation.');
                        returnObject.processSuccessful = true;
                        returnObject.rebalancingProcessSuccess = false;
                ...
                if ((inputObject.additionalLumpsumBuy > 0 && isLumpsum) || (inputObject.additionalSIPBuy > 0 && !isLumpsum)) {
                    //continue further to do buy
                    logging_utils_1.LoggingUtils.info('Additional buy journey. We will process the additional buy needed.');
                }
                else {
                    logging_utils_1.LoggingUtils.info('Neither rebalancing asked nor any additional Buy is there. INVALID CASE of inputs.');
                    logging_utils_1.LoggingUtils.info('Abe bacche ki jaan lega kya !!! :))))');
            ...
            assetClassTotal = goal_utils_1.GoalUtils.sumProduct(portfolioData, 'weightinIteration', assetClasses, categoryAssetExposure, 'category', 'initialNetAssetWeights');
            if (Math.abs(1 - getArraySum(assetClassTotal)) < precisionPoint) {
                logging_utils_1.LoggingUtils.info('Successfully completed rebalancing !!!');
                returnObject.processSuccessful = true;
                ...
                if (Math.abs(1 - getArraySum(assetClassTotal)) < waterfallTolerance) {
                    logging_utils_1.LoggingUtils.info('Successfully completed rebalancing with some buy remaining for minumum amounts !!!');
                    returnObject.processSuccessful = true;
                    returnObject.rebalancingProcessSuccess = true;
                    ...
                    logging_utils_1.LoggingUtils.info('Something wrong... ERROR !!!');
        ...
        function initialize() {
            ...
            let origPortfolioTotalAmount = 0;
            ...
            logging_utils_1.LoggingUtils.debug(`appending ModelPortfolio: ${JSON.stringify(modelPortfolio)}`);
            appendModelRowstoPortfolio(portfolioTotalAmount);
            logging_utils_1.LoggingUtils.debug(`done appending: ${JSON.stringify(portfolioData)}`);
            ...
            modelPortfolioAllocations = goal_utils_1.GoalUtils.setModelPortfolio(tenureMonths, totalAmount, riskProfileId, isLumpsum);
            logging_utils_1.LoggingUtils.info('done modelPortfolioAllocations');
            ...
            for (let i = 0; i < portfolioData.length; i++) {
                ...
                portfolioData[i]['freeUnits'] = Math.min(portfolioData[i]['freeUnits'], portfolioData[i]['weight'] - portfolioData[i]['lockedUnits']);
                //general columns
                portfolioData[i]['initialNetAssetWeights'] = goal_utils_1.GoalUtils.createWtArray(assetClasses);
                portfolioData[i]['initialNetCategoryWeights'] = goal_utils_1.GoalUtils.createWtArray(categories);
                portfolioData[i]['initialGroupCategoryWeights'] = goal_utils_1.GoalUtils.createWtArray(groupCategories);
                //sell related columns
                portfolioData[i]['canProcess'] = false;
                portfolioData[i]['initialWeight'] = portfolioData[i]['weight'];
                portfolioData[i]['groupCategoryWeightstoFulfill'] = goal_utils_1.GoalUtils.createWtArray(groupCategories);
            ...
            logging_utils_1.LoggingUtils.info('done portfolioData initialization');
            ...
            assetClassTotal = goal_utils_1.GoalUtils.sumProduct(portfolioData, 'originalWeight', assetClasses, categoryAssetExposure, 'category', 'initialNetAssetWeights');
            let portfolioAC = {};
            ...
            assetClassTotal = goal_utils_1.GoalUtils.sumProduct(portfolioData, 'weight', assetClasses, categoryAssetExposure, 'category', 'initialNetAssetWeights');
            ...
            categoryGroupTotal = goal_utils_1.GoalUtils.sumProduct(portfolioData, 'weight', groupCategories, categoryCategoryGroupExposure, 'category', 'initialGroupCategoryWeights');
            logging_utils_1.LoggingUtils.info('done totals');
            //get the deviations at Asset Class level
            assetClassDeviationsTotal = goal_utils_1.GoalUtils.getAggregateDeviations(assetClasses, assetClassesNames, assetClassTotal, 'assetClassLevel', 'assetClass', 'deviation', modelPortfolioAllocations);
            //get deviations at Category Group level
            categoryGroupDeviationsTotal = goal_utils_1.GoalUtils.getAggregateDeviations(groupCategories, groupCategoriesNames, categoryGroupTotal, 'categoryGroupLevel', 'categoryGroup', 'deviation', modelPortfolioAllocations);
            logging_utils_1.LoggingUtils.info('done aggregations');
            logging_utils_1.LoggingUtils.info(`assetClassTotal: , ${JSON.stringify(assetClassTotal)}`);
            logging_utils_1.LoggingUtils.info(`categoryGroupTotal: , ${JSON.stringify(categoryGroupTotal)}`);
            logging_utils_1.LoggingUtils.info(`assetClassDeviationsTotal : , ${JSON.stringify(assetClassDeviationsTotal)}`);
            logging_utils_1.LoggingUtils.info(`categoryGroupDeviationsTotal : , ${JSON.stringify(categoryGroupDeviationsTotal)}`);
        }
        function getMaxSchemeWeight(categoryGroupId, mpMiniFull) {
            ...
            if (!categoryGroupWeight) {
                logging_utils_1.LoggingUtils.info(`Non reco category max weight could not be found. Setting  ${maxWeight * 100}, '% by default`);
                return maxWeight;
            }
            let maxSchemeWeights = constants_1.Option.GLOBALOPTIONS.MAXSHCEMEWEIGHTSETTING[mpMiniFull].maxSchemeWeight;
            let categoryWeights = constants_1.Option.GLOBALOPTIONS.MAXSHCEMEWEIGHTSETTING[mpMiniFull].categoryWeight;
            ...
            if (foundIndex == -1) {
                logging_utils_1.LoggingUtils.info('Error: max weight could not be found. Setting ', maxWeight * 100 + `% by default`);
                //maxWeight = 0.3;
        ...
        function appendModelRowstoPortfolio(portfolioTotalAmount) {
            ...
            logging_utils_1.LoggingUtils.info('in appendModelRowstoPortfolio');
            let holdings = modelPortfolio;
            if (holdings) {
                logging_utils_1.LoggingUtils.info(`appending modelPortfolio data with rows = , ${modelPortfolio.length}`);
                for (let i = 0; i < holdings.length; i++) {
                    let obj = portfolioData.find((item) => item.instrumentId === holdings[i].instrumentId);
                    ...
                    currHoldingRow['isRecoScheme'] = goal_utils_1.GoalUtils.getRecommendationTypeLabel(recommendationType, defaultReturnLabel);
                    ...
                    if (isLumpsum) {
                        ...
                        let currentSIPSetting = holdings[i].currentSIPSetting;
        ...
        function processBuywithPriorityLoops() {
            ...
            let totalBuysDone = goal_utils_1.GoalUtils.sumProduct(portfolioData, 'buyProcessed', assetClasses, categoryAssetExposure, 'category', 'initialNetAssetWeights');
            //NOTE2: Reverting
            assetClassTotal = goal_utils_1.GoalUtils.sumProduct(portfolioData, 'weightinIteration', assetClasses, categoryAssetExposure, 'category', 'initialNetAssetWeights');
            let totalCurrWeight = 0;
            let totalBought = 0;
            ...
            logging_utils_1.LoggingUtils.info(`totalCurrWeight, totalBought, maxBuy: ', ${totalCurrWeight}, ${totalBought}, ${maxBuy}`);
            ...
            while (buyDone != 0) {
                ...
                logging_utils_1.LoggingUtils.info(`currAssetWaterfall,${JSON.stringify(currAssetWaterfall)}`);
                //3. get categoryGroup to buy using waterfall, within the assetClass selected
                let currCategoryGroupWaterfall = {};
                ...
                logging_utils_1.LoggingUtils.info(`currCategoryGroupWaterfall, ${JSON.stringify(currCategoryGroupWaterfall)}`);
                ...
                for (let i = 0; i < buyPriorityArray.length; i++) {
                    ...
                    logging_utils_1.LoggingUtils.info(`Processing buys with P, ${i + 1}`);
                    ...
                    if (Math.abs(buyDoneinCategoryGroup - currGroupCategoryBuyNeeded) < precisionPoint) {
                        logging_utils_1.LoggingUtils.info(`Done buy for categoryGroup total: , ${buyDoneinCategoryGroup}`);
                        break;
                    }
                }
                buyDone = buyDoneinCategoryGroup;
                assetClassTotal = goal_utils_1.GoalUtils.sumProduct(portfolioData, 'weightinIteration', assetClasses, categoryAssetExposure, 'category', 'initialNetAssetWeights');
                logging_utils_1.LoggingUtils.info(`After processing assetWaterfall: ', ${assetClassTotal}, ${getArraySum(assetClassTotal)}`);
                if (Math.abs(1 - getArraySum(assetClassTotal)) < precisionPoint) {
                    logging_utils_1.LoggingUtils.info('Buys completed.');
                    return;
                }
                else {
                    if (buyDone == 0) {
                        logging_utils_1.LoggingUtils.info(`Will exit this asset class as it's not able to buy for min amount = , ${currAssetWaterfall.buyValue}`);
                        logging_utils_1.LoggingUtils.info(`portfolioData ', ${JSON.stringify(portfolioData)}`);
                    }
                }
            }
        }
        function buyGroupCategory(currGroupCategoryObject, maxBuyColumn, buyDoneinCategoryGroup) {
            ...
            //sort then by rank
            setProcessingOrder(portfolioData, 'groupCategoryWeightstoFulfill', 'ascending', true, groupCategories.indexOf(currGroupCategory), 'schemeRank', 'ascending');
            //LoggingUtils.info("in buyGroupCategory");
            //LoggingUtils.info(portfolioData);
            let totalBuyDone = 0;
            logging_utils_1.LoggingUtils.info(`Starting loop for currGroupCategory: , ${currGroupCategory}`);
            //loop through and sell
            for (let i = 0; i < portfolioData.length; i++) {
                logging_utils_1.LoggingUtils.info(`Trying with instrument: , ${portfolioData[i]['instrumentName']}`);
                ...
                logging_utils_1.LoggingUtils.info(`Buy needed from this instrument - , ${buyNeeded}`);
                ...
                if (buyNeeded > 0 && buyCurrIteration > 0) {
                    logging_utils_1.LoggingUtils.info(`buyCurrIteration, totalBuyDone :, ${buyCurrIteration}, ${totalBuyDone}`);
                    let isSell = false;
                    ...
                    logging_utils_1.LoggingUtils.info(`buyCurrIterationInstrumentCheck :, ${JSON.stringify(buyCurrIterationInstrumentCheck)}`);
                    if (buyCurrIteration > 0) {
                        ...
                        logging_utils_1.LoggingUtils.info(`Bought: , portfolioData[i]['instrumentId'] - , ${buyCurrIteration}, ', Rs. ', ${buyCurrIterationAmount}`);
                    }
        ...
        function calculateInstrumentBuySellNeeded(maxColumn, isSell) {
            ...
            categoryGroupTotal = goal_utils_1.GoalUtils.sumProduct(portfolioData, 'weightinIteration', groupCategories, categoryCategoryGroupExposure, 'category', 'initialGroupCategoryWeights');
            logging_utils_1.LoggingUtils.info(`currCategoryGroupWaterfall,${JSON.stringify(categoryGroupTotal)}`);
            for (let i = 0; i < portfolioData.length; i++) {
                //initialize
                portfolioData[i]['groupCategoryWeightstoFulfill'] = goal_utils_1.GoalUtils.createWtArray(groupCategories);
        ...
        function sellNonRecommendedSchemes() {
            for (let i = 0; i < portfolioData.length; i++) {
                ...
                if (portfolioData[i]['isRecoScheme'] == 'SELL') {
                    ...
                    logging_utils_1.LoggingUtils.info(`Sold not recommended : ',
        ...
        function processSell() {
            ...
            if (cleanUpNotRecommendedSchemes) {
                ...
                logging_utils_1.LoggingUtils.info('******* Non recommended Sell Done *******');
                categoryGroupTotal = goal_utils_1.GoalUtils.sumProduct(portfolioData, 'weightinIteration', groupCategories, categoryCategoryGroupExposure, 'category', 'initialGroupCategoryWeights');
                categoryGroupDeviationsTotal = goal_utils_1.GoalUtils.getAggregateDeviations(groupCategories, groupCategoriesNames, categoryGroupTotal, 'categoryGroupLevel', 'categoryGroup', 'deviation', modelPortfolioAllocations);
                logging_utils_1.LoggingUtils.info(`Category Group Deviations after Non recommended sell: , ${JSON.stringify(categoryGroupDeviationsTotal)}`);
                assetClassTotal = goal_utils_1.GoalUtils.sumProduct(portfolioData, 'weightinIteration', assetClasses, categoryAssetExposure, 'category', 'initialNetAssetWeights');
                assetClassDeviationsTotal = goal_utils_1.GoalUtils.getAggregateDeviations(assetClasses, assetClassesNames, assetClassTotal, 'assetClassLevel', 'assetClass', 'deviation', modelPortfolioAllocations);
                logging_utils_1.LoggingUtils.info(`Asset class Deviations after Non recommended sell: , ${JSON.stringify(assetClassDeviationsTotal)}`);
            }
            //1A. Sell of
            //a. Non-reco Category: isRecoCat = 0
            //b. freeUnits
            logging_utils_1.LoggingUtils.info('Selling P1A');
            processSellwithPriority([['isRecoCat', '==', 0]], 'freeUnits');
            //1B. Sell of
            //a. Non-reco Category: isRecoCat = 0
            //b. allUnits: maxSellPossible
            logging_utils_1.LoggingUtils.info('Selling P1B');
            processSellwithPriority([['isRecoCat', '==', 0]], 'maxSellPossible');
            //2A. Sell of
            //a. Non-reco schemes: isRecoScheme = "SELL"
            //b. freeUnits
            logging_utils_1.LoggingUtils.info('Selling P2A');
            processSellwithPriority([['isRecoScheme', '==', 'SELL']], 'freeUnits');
            //2B. Sell of
            //a. Sell Schemes: isRecoScheme = "SELL"
            //b. allUnits: maxSellPossible
            logging_utils_1.LoggingUtils.info('Selling P2B');
            processSellwithPriority([['isRecoScheme', '==', 'SELL']], 'maxSellPossible');
            //3A. Sell of
            //a. Hold Schemes: isRecoScheme = "HOLD"
            //b. freeUnits
            logging_utils_1.LoggingUtils.info('Selling P3A');
            processSellwithPriority([['isRecoScheme', '==', 'HOLD']], 'freeUnits');
            //3B. Sell of
            //a. Hold Schemes: isRecoScheme = "HOLD"
            //b. allUnits: maxSellPossible
            logging_utils_1.LoggingUtils.info('Selling P3B');
            processSellwithPriority([['isRecoScheme', '==', 'HOLD']], 'maxSellPossible');
            //4A. Sell of
            //a. Buy Schemes: isRecoScheme = "BUY"
            //b. freeUnits
            logging_utils_1.LoggingUtils.info('Selling P4A');
            processSellwithPriority([['isRecoScheme', '==', 'BUY']], 'freeUnits');
            //4B. Sell of
            //a. Buy Schemes: isRecoScheme = "BUY"
            //b. allUnits: maxSellPossible
            logging_utils_1.LoggingUtils.info('Selling P4B');
            processSellwithPriority([['isRecoScheme', '==', 'BUY']], 'maxSellPossible');
            logging_utils_1.LoggingUtils.info('******* Category Group Sell Done *******');
            categoryGroupTotal = goal_utils_1.GoalUtils.sumProduct(portfolioData, 'weightinIteration', groupCategories, categoryCategoryGroupExposure, 'category', 'initialGroupCategoryWeights');
            categoryGroupDeviationsTotal = goal_utils_1.GoalUtils.getAggregateDeviations(groupCategories, groupCategoriesNames, categoryGroupTotal, 'categoryGroupLevel', 'categoryGroup', 'deviation', modelPortfolioAllocations);
            logging_utils_1.LoggingUtils.info(`Category Group Deviations after sell: ',${JSON.stringify(categoryGroupDeviationsTotal)} `);
            assetClassTotal = goal_utils_1.GoalUtils.sumProduct(portfolioData, 'weightinIteration', assetClasses, categoryAssetExposure, 'category', 'initialNetAssetWeights');
            assetClassDeviationsTotal = goal_utils_1.GoalUtils.getAggregateDeviations(assetClasses, assetClassesNames, assetClassTotal, 'assetClassLevel', 'assetClass', 'deviation', modelPortfolioAllocations);
            ...
            logging_utils_1.LoggingUtils.info(`Asset class Deviations after sell: , ${JSON.stringify(assetClassDeviationsTotal)}`);
        ...
        function doAssetClassSellProcessing() {
            logging_utils_1.LoggingUtils.info('****** Deviations at assetClass exist even after categoryGroup sell.');
            logging_utils_1.LoggingUtils.info('****** This may happen when something at category level was locked and didnt allow the sell needed.');
            logging_utils_1.LoggingUtils.info('****** Now selling by assetClass. So CategoryGroup deviations may remain/enhanced.');
            logging_utils_1.LoggingUtils.info('****** Asset class deviation is higher priority.');
            ...
            for (let i = 0; i < assetClasses.length; i++) {
                ...
                if (currAssetClassDeviation < 0 && Math.abs(currAssetClassDeviation) >= precisionPoint) {
                    ...
                    logging_utils_1.LoggingUtils.info(`Sold Asset class: ', ${JSON.stringify(currAssetClass)}, '::', ${JSON.stringify(currAssetClassName)}, '::', ${JSON.stringify(soldAssetClassValue)}`);
                    if (Math.abs(currAssetClassDeviation - soldAssetClassValue) < precisionPoint) {
                        logging_utils_1.LoggingUtils.info('Achieved asset class alignment via Asset class sell');
                    }
                    else {
                        logging_utils_1.LoggingUtils.info('Could NOT achieve asset class alignment via Asset class sell');
                    }
                }
            }
        }
        function sellAssetClass(currAssetClass, currAssetClassName, currAssetClassDeviation) {
            logging_utils_1.LoggingUtils.info(`Trying to sell Asset class: , ${JSON.stringify(currAssetClass)}, '::', ${JSON.stringify(currAssetClassName)}, '::', ${JSON.stringify(currAssetClassDeviation)}`);
            ...
            for (let i = 0; i < portfolioData.length; i++) {
                ...
                logging_utils_1.LoggingUtils.info(`Sold: ', ${portfolioData[i]['instrumentId']}, ' - ', ${sellCurrIteration}, ', Rs. ', ${sellCurrIterationAmount}`);
        ...
        function processSellwithPriority(colConditionArray, maxSellColumn) {
            ...
            if (categoryGroupDeviationsTotal[0]['deviation'] < -1 * precisionPoint) {
                ...
                //sell done
                logging_utils_1.LoggingUtils.info('sell is Done...');
                return true;
            }
            //3. put what can be sold or not depending on iteration and priority
            setInstrumentProcessFlag(colConditionArray);
        ...
        function doSellLoops(maxSellColumn) {
            ...
            for (let i = 0; i < categoryGroupDeviationsTotal.length; i++) {
                ...
                if (categoryGroupDeviationsTotal[i]['deviation'] < -1 * precisionPoint) {
                    logging_utils_1.LoggingUtils.info(`Selling categoryGroup:',
        ...
        function sellGroupCategory(currGroupCategory, maxSellColumn) {
            ...
            setProcessingOrder(portfolioData, 'groupCategoryWeightstoFulfill', 'descending', true, groupCategories.indexOf(currGroupCategory), 'schemeRank', 'descending');
            //LoggingUtils.info(portfolioData);
            //loop through and sell
            for (let i = 0; i < portfolioData.length; i++) {
        ...
        function buySellCheckforInstrument(isLumpsum, isSell, processCurrIteration, portfolioDataRow) {
            //Convert % to amount
        ...
        function getMinMaxTransaction(isLumpsum, isSell, hasInvestment, portfolioDataRow) {
        ...
        function setProcessingOrder(objectArray, sortProperty, order, sortPropertyisArray = false, arrayIndexToSort = 0, sortSecondProperty = '', orderSecond = 'ascending', sortSecondPropertyisArray = false, arrayIndexToSortSecond = 0) {
        ...
        function getBuyAssetClasswithWaterfall() {
            //get total buys done
            //NOTE1: We are using buyProcessed column for totalBuys done which shall disturb expected values in output initialNetAssetWeights column
            //So we shall revert immediately once we have the data calculated
            let totalBuysDone = goal_utils_1.GoalUtils.sumProduct(portfolioData, 'buyProcessed', assetClasses, categoryAssetExposure, 'category', 'initialNetAssetWeights');
            ...
            let buyAssetObject = {};
            if (maxBuy < precisionPoint) {
                logging_utils_1.LoggingUtils.info('All buy done.');
        ...
        function getBuyCategoryGroupwithWaterfall(buyAssetObject) {
            let processAsset = '';
            ...
            if (Object.keys(buyAssetObject).length === 0) {
                ...
                logging_utils_1.LoggingUtils.info('Doing categoryGroup waterfall for all categoryGroups');
            }
            else {
                //
                ...
                logging_utils_1.LoggingUtils.info(`Doing categoryGroup waterfall ONLY for assetClass in ', ${processAsset}, ' : ', ${processAssetIndex}`);
            }
            ...
            for (let j = 0; j < categoryGroupDeviationsTotal.length; j++) {
                ...
                if (processAsset == '') {
                    ...
                    logging_utils_1.LoggingUtils.info(`tempCategoryGroup, processAssetIndex, deviation --- ',
        ...
        function getBuyObjectWithWaterfall(inputArray, keyName, maxBuy) {
        ...
        function setInstrumentProcessFlag(colConditionArray) {
            for (let i = 0; i < portfolioData.length; i++) {
                //initialize
                portfolioData[i]['canProcess'] = false;
        ...
        function dynamicCompare(post, operator, value) {
            switch (operator) {
                case '>':
                    return post > value;
                case '<':
...
exports.RebalancingUtils = RebalancingUtils;
//# sourceMappingURL=rebalancing-utils.js.map