...
export class CoreBankingFacade {
    ...
    try {
      ...
      if (
        ...
        if (!checkUniqueUser) {
          ...
          if (userId) {
            if (!element?.refCustItNum) {
              await this.appUserRepository.updateAll({
                panAadharLinkStatus: Option.GLOBALOPTIONS.PANAADHARLINKSTATUS.notFound.value
              }, {
                id: userId
              })
              return Promise.reject(new RestError(400, 'PAN not found!', {systemcode: 1398}));
            }
          }
                  ...
                  and: [
                    {
                      birthDate:
                        ...
                        (element?.datBirthCust && moment(element.datBirthCust).format('YYYY-MM-DD')),
                      appUserId: userId
                    }
                  ]
                },
  ...
  async getOTP(transactionId: string, linkData: string) {
    try {
      const otpResponse: any = await this.coreBankingRepository.doGenerateOTP(transactionId, linkData);
