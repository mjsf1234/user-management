  ...
  async updateBankAccount(id: number, accountId: number, data: DataObject<BankAccount>, options?: Options): Promise<Object> {
    return new Promise((resolve, reject) => {
      ...
      this.bankAccountRepository
        ...
        .then((bankAccount: BankAccount | null) => {
          ...
          if (bankAccountData.account.primaryHolderId) {
            if (!bankAccountData.account.primaryHolder.bosCode) {
              return Promise.reject(new RestError(400, 'We regret to inform you that we are unable to proceed with your request, as your PAN and Aadhaar details couldn’t be retrieved.', {systemcode: 1397}));
            }
            // validate pan and aadhar linking
            return this.validatePanAadharLink(bankAccountData.account.primaryHolder.bosCode);
          } else {
            return Promise.reject(new RestError(400, 'Primary User not found', {systemcode: 1397}));
          }
        })
        .then((panAadharLinkStatus: number) => {
        ...
        .then(() => {
          if (panAadharLinkStatusG === Option.GLOBALOPTIONS.PANAADHARLINKSTATUS.nonOperative.value) {
            return Promise.reject(new RestError(400, 'We regret to inform you that we are unable to proceed with your request, as your PAN and Aadhaar are not linked. To link your PAN with Aadhaar: https://eportal.incometax.gov.in/iec/foservices/#/pre-login/bl-link-aadhaar', {systemcode: 1396}));
          }
          if (panAadharLinkStatusG === Option.GLOBALOPTIONS.PANAADHARLINKSTATUS.pending.value) {
            return Promise.reject(new RestError(400, 'We regret to inform you that we are unable to proceed with your request, as your PAN and Aadhaar details couldn’t be retrieved.', {systemcode: 1397}));
          }
