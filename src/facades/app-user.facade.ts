...
export class AppUserFacade {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.deviceRepository
        ...
        .then(async (device: Device | null) => {
          ...
          return Promise.reject(new RestError(400, 'Invalid MPIN.', {systemcode: 1024}));
        })
        .then(async (device: Device | null) => {
    ...
    try {
      ...
      if (!(panAndDOBDetails && (panAndDOBDetails.PAN || panAndDOBDetails.DOB) && panAndDOBDetails.deviceId)) {
        ...
        if (etbDetails!.success) {
          ...
          let updateUserStatus = await this.appUserRepository.updateAll(
            {appUserStatus: Option.GLOBALOPTIONS.APPUSERSTATUS.NTBUser.value, panAadharLinkStatus: Option.GLOBALOPTIONS.PANAADHARLINKSTATUS.notFound.value},
            {id: appUserId, isActive: true}
          );
          if (!updateUserStatus) {
            //@todo add it to error logs
  ...
  async getOTP(transactionId: string, linkData: string) {
    try {
  ...
  async getOTPMock(transactionId: string, linkData: string) {
  ...
  async doPublishOTP(contactNumber: string, otp: OtpMessages, transactionId: string) {
      ...
      if (serviceProviderData?.serviceProvider.rta?.id == 2) {
        ...
        if (
          ...
          await this.serviceProviderAccountRepository.updateAll(
            {
              registeredMobile: response?.Services?.Details?.Investor_Details[0]?.mobileno ? response?.Services?.Details?.Investor_Details[0]?.mobileno : null,
              registeredEmail: response?.Services?.Details?.Investor_Details[0]?.email ? response?.Services?.Details?.Investor_Details[0]?.email : null,
              ContactDetailsUpdatedOn: new Date()
            },
            {id: serviceProviderAccountID},
            Options
          );
          return {email: response?.Services?.Details?.Investor_Details[0]?.email ? response?.Services?.Details?.Investor_Details[0]?.email : null, mobile: response?.Services?.Details?.Investor_Details[0]?.mobileno ? response?.Services?.Details?.Investor_Details[0]?.mobileno : null};
        } else {
          if (serviceProviderData?.registeredEmail || serviceProviderData?.registeredMobile) {
            return {email: serviceProviderData?.registeredEmail, mobile: serviceProviderData?.registeredMobile};
          } else {
        ...
        if (response?.Email || response?.Mobile || response?.Email == '' || response?.Mobile == '') {
          await this.serviceProviderAccountRepository.updateAll(
            {registeredMobile: response.Mobile ? response.Mobile : null, registeredEmail: response.Email ? response.Email : null, ContactDetailsUpdatedOn: new Date()},
            {id: serviceProviderAccountID},
            Options
          );
