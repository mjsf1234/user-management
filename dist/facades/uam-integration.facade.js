...
exports.UamIntegrationFacade = void 0;
...
const xml_js_1 = (0, tslib_1.__importDefault)(require("xml-js"));
const MAX_DATA_FETCH_LIMIT = 30;
const MIN_SEARCH_CHAR_LIMIT = 3;
// All business loigc goes inside Facade layer
let UamIntegrationFacade = class UamIntegrationFacade {
    constructor(uamIntegrationRepository, appRoleRepository, appUserRepository, uamLogRepository, appUserRoleMappingRepository, operationRepository, uamLoginAttemptsConfigRepository, appAccessTokenRepository, roleRightsRepository) {
        this.uamIntegrationRepository = uamIntegrationRepository;
        this.appRoleRepository = appRoleRepository;
        this.appUserRepository = appUserRepository;
        this.uamLogRepository = uamLogRepository;
    ...
    async isValidRequest(type, xmlToJsonObj) {
    ...
    async xmlResponse(body, previous = false) {
        try {
            if (typeof body == 'string')
                body = JSON.parse(body);
            const jsonBody = { Response: body };
            const stringifiedBody = JSON.stringify(jsonBody);
            const response = xml_js_1.default.json2xml(stringifiedBody, { compact: true, ignoreComment: true, spaces: 4 });
    ...
    async parseSoapToJSONParam(index, xmlToJsonObj) {
        let response = {};
        underscore_1.default.each(xmlToJsonObj['s:envelope']['s:body'][0][index][0].in[0], (item, param) => {
            response[this.parameterize(param)] = item[0]._ ? item[0]._.trim() : item[0]._;
        });
        return response;
    }
    async isDuplicateRequest(uniqueNumber) {
    ...
    //     });
    //   }
    async saveNonRMUserDetails(req, details, appRoleId) {
        try {
            if (!details) {
                return Promise.reject(new common_1.RestError(400, `Details is required!`, { systemcode: 1329 }));
            }
            if (!details.name) {
                return Promise.reject(new common_1.RestError(400, `Name is required!`, { systemcode: 1330 }));
            }
            if (!details.userCode) {
                return Promise.reject(new common_1.RestError(400, `Invalid Employee Code!`, { systemcode: 1331 }));
            }
            if (details.dob && !new Date(details.dob)) {
                return Promise.reject(new common_1.RestError(400, `Invalid Date of Birth!`, { systemcode: 1332 }));
            }
            let userDetails;
            const isUserExist = await this.appUserRepository.findOne({
                where: {
                    // name: details.name,
                    // email: details.email,
            ...
            if (isUserExist) {
                return Promise.reject(new common_1.RestError(400, 'User already Exists!', { systemcode: 1333 }));
            }
            const hashPassword = bcrypt.hashSync('demo', salt);
            let userObj = {
                appUserStatus: common_1.Option.GLOBALOPTIONS.APPUSERSTATUS['pendingRegistration'].value,
                name: details.name,
                email: details.email,
    ...
    async createUserUsingUAM(requestData) {
        try {
            let appRoleInstance, jsonObject, appUserInstance = {}, isacUsers;
            ...
            const xmlToJsonObj = xml_js_1.default.xml2js(requestData, { compact: true, sanitize: true });
            ...
            if (previousResponse && (previousResponse === null || previousResponse === void 0 ? void 0 : previousResponse.responseObject)) {
                return this.xmlResponse(previousResponse === null || previousResponse === void 0 ? void 0 : previousResponse.responseObject, true);
            }
            jsonObject.activity = 'ADD';
            const emailParts = jsonObject.emailId.split('@');
            if (!jsonObject.emailId ||
                !common_1.ValidationUtils.validateEmail(jsonObject.emailId) ||
            ...
            if (jsonObject.dateOfBirth && !(0, moment_timezone_1.default)(jsonObject.dateOfBirth, 'DDMMYYYY').isValid()) {
                return this.xmlResponse({ message: 'INVALID DATE OF BIRTH', statusCode: 7, uniqueNumber: jsonObject.uniqueNumber });
            }
            if (jsonObject.contactNumber && !common_1.ValidationUtils.validateMobileNumber(jsonObject.contactNumber)) {
            ...
            if (rawAppUser &&
                rawAppUser.length > 0 &&
                rawAppUser[0]['appUserStatus'] != common_1.Option.GLOBALOPTIONS.APPUSERSTATUS['pendingRegistration'].value) {
                return this.xmlResponse({ message: 'USERID/EMPCODE ALREADY EXISTS!', statusCode: 2, uniqueNumber: jsonObject.uniqueNumber });
            }
            let dobToSave = jsonObject.dateOfBirth ? moment_timezone_1.default.utc(jsonObject.dateOfBirth, 'DDMMYYYY').format('YYYY-MM-DD') : null;
            ...
            appUserInstance.appUserStatus = common_1.Option.GLOBALOPTIONS.APPUSERSTATUS['active'].value;
            ...
            let relatedData;
            if (appUserInstance.operationDetails && appUserInstance.operationDetails.userType == common_1.Option.GLOBALOPTIONS.USERTYPE['general'].value) {
            ...
            const createUAMIntegration = await this.create({
                ...
                status: common_1.Option.GLOBALOPTIONS.APPUSERSTATUS['active'].label,
                appUserId: appUserInstance.id,
                isLatest: true,
                profile: appRoleInstance.name,
            ...
            common_1.LoggingUtils.info(UAMLogs, 'User Created');
    ...
    async disableUserUsingUAM(requestData) {
        try {
            const xmlToJsonObj = xml_js_1.default.xml2js(requestData, { compact: true, sanitize: true });
            let jsonObject = {};
            jsonObject = xmlToJsonObj;
            ...
            let currentAppUserStatus = await this.returnAppUserStatusLabel(appUserInstance.appUserStatus);
            if (appUserInstance.appUserStatus === common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.disabled.value) {
                return this.xmlResponse({ message: 'USERID ALREADY DISABLE', statusCode: 11, uniqueNumber: jsonObject.uniqueNumber });
            }
            if (appUserInstance.appUserStatus === common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.deleted.value) {
                return this.xmlResponse({ message: 'USERID IS DELETED CANNOT BE MODIFIED', statusCode: 18, uniqueNumber: jsonObject.uniqueNumber });
            }
            if (!(appUserInstance.appUserStatus == common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.locked.value ||
                appUserInstance.appUserStatus == common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.dormant.value ||
                appUserInstance.appUserStatus == common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.active.value)) {
            ...
            appUserInstance.appUserStatus = common_1.Option.GLOBALOPTIONS.APPUSERSTATUS['disabled'].value;
            ...
            if (uamLog) {
                ...
                UAMData.status = common_1.Option.GLOBALOPTIONS.APPUSERSTATUS['disabled'].label;
                ...
                common_1.LoggingUtils.info(UAMLogs, 'User Disabled');
            }
            await this.appAccessTokenRepository.updateAll({ isActive: false }, { isActive: true, appUserId: appUser === null || appUser === void 0 ? void 0 : appUser.id });
            return this.xmlResponse({ message: 'success', statusCode: 0, uniqueNumber: jsonObject.uniqueNumber });
        }
        catch (err) {
            return this.xmlResponse({ message: 'Failure', statusCode: 1, err });
    ...
    async enableUserUsingUAM(requestData) {
        try {
            let appUserInstance;
            const xmlToJsonObj = xml_js_1.default.xml2js(requestData, { compact: true, sanitize: true });
            let jsonObject = {};
            jsonObject = xmlToJsonObj;
            let changedFromDormant = false;
            ...
            let currentAppUserStatus = await this.returnAppUserStatusLabel(appUser.appUserStatus);
            ...
            if (appUser.appUserStatus != common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.disabled.value && appUser.appUserStatus != common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.dormant.value) {
                return this.xmlResponse({
                    message: `USER IS CURRENTLY IN ${currentAppUserStatus} STATE`,
                    statusCode: 10,
                    uniqueNumber: jsonObject.uniqueNumber
            ...
            if (appUser.appUserStatus == common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.dormant.value)
                changedFromDormant = true; // We are setting this to mark if the user was enabled from dormant status recently
            ...
            if (uamLog) {
                ...
                UAMData.dormantDateTime = null;
                UAMData.deletionDateTime = null;
                UAMData.status = common_1.Option.GLOBALOPTIONS.APPUSERSTATUS['active'].label;
                UAMData.lastLoginDate = jsonObject.lastLoginDate;
                UAMData.activity = jsonObject.activity;
                ...
                common_1.LoggingUtils.info(UamLogs, 'User enabled');
            }
            return this.xmlResponse({ message: 'success', statusCode: 0, uniqueNumber: jsonObject.uniqueNumber });
        }
        catch (err) {
    ...
    async deleteUserUsingUAM(requestData) {
        try {
            let appUserInstance;
            const xmlToJsonObj = xml_js_1.default.xml2js(requestData, { compact: true, sanitize: true });
            let jsonObject = {};
            ...
            if (appUser.appUserStatus === common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.deleted.value) {
                return this.xmlResponse({ message: 'USERID ALREADY DELETED', statusCode: 16, uniqueNumber: jsonObject.uniqueNumber });
            }
            appUserInstance = appUser;
            jsonObject.lastLoginDate = appUser.lastLoginDate;
            appUserInstance.previousStatus = appUserInstance.appUserStatus;
            appUserInstance.appUserStatus = common_1.Option.GLOBALOPTIONS.APPUSERSTATUS['deleted'].value;
            appUserInstance.isActive = false;
            appUserInstance.lastModifiedDate = new Date();
            const createAppUser = await this.appUserRepository.updateAll(appUserInstance, { id: appUserInstance.id });
            if (!createAppUser) {
            ...
            if (uamLog) {
                ...
                UAMData.newGender = null;
                UAMData.disableDateTime = null;
                UAMData.status = common_1.Option.GLOBALOPTIONS.APPUSERSTATUS['deleted'].label;
                UAMData.lastLoginDate = jsonObject.lastLoginDate;
                UAMData.activity = jsonObject.activity;
                ...
                common_1.LoggingUtils.info(UamLogs, 'User deleted');
            }
            await this.appAccessTokenRepository.updateAll({ isActive: false }, { isActive: true, appUserId: appUser === null || appUser === void 0 ? void 0 : appUser.id });
            return this.xmlResponse({ message: 'success', statusCode: 0, uniqueNumber: jsonObject.uniqueNumber });
        }
    ...
    async reopenUserUsingUAM(requestData) {
        try {
            let appUserInstance;
            const xmlToJsonObj = xml_js_1.default.xml2js(requestData, { compact: true, sanitize: true });
            let jsonObject = {};
            ...
            if (appUser.appUserStatus == common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.disabled.value) {
                return this.xmlResponse({
                    message: `USER IS CURRENTLY IN ${currentAppUserStatus} STATE`,
                    statusCode: 20,
                    uniqueNumber: jsonObject.uniqueNumber
            ...
            if (appUser.appUserStatus == common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.locked.value) {
                return this.xmlResponse({
                    message: `USER IS CURRENTLY IN ${currentAppUserStatus} STATE`,
                    statusCode: 19,
                    uniqueNumber: jsonObject.uniqueNumber
            ...
            if (appUser.appUserStatus == common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.suspended.value) {
                return this.xmlResponse({
                    message: `USER IS CURRENTLY IN ${currentAppUserStatus} STATE`,
                    statusCode: 21,
                    uniqueNumber: jsonObject.uniqueNumber
            ...
            if (uamLog) {
                ...
                common_1.LoggingUtils.info(UamLogs, 'User re-opened');
            }
            return this.xmlResponse({ message: 'success', statusCode: 0, uniqueNumber: jsonObject.uniqueNumber });
        }
        catch (err) {
    ...
    async unlockUserUsingUAM(requestData) {
        try {
            let appUserInstance;
            const xmlToJsonObj = xml_js_1.default.xml2js(requestData, { compact: true, sanitize: true });
            let jsonObject = {};
            ...
            let currentAppUserStatus = await this.returnAppUserStatusLabel(appUserInstance.appUserStatus);
            if (appUserInstance && appUserInstance.appUserStatus == common_1.Option.GLOBALOPTIONS.APPUSERSTATUS['active'].value) {
            ...
            if (appUserInstance.appUserStatus === common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.suspended.value) {
                return this.xmlResponse({
                    message: `USER IS CURRENTLY IN ${currentAppUserStatus} STATE`,
                    statusCode: 21,
                    uniqueNumber: jsonObject.uniqueNumber
            ...
            if (uamLog) {
                ...
                common_1.LoggingUtils.info(UamLogs, 'User unlocked');
            }
            return this.xmlResponse({ message: 'success', statusCode: 0, uniqueNumber: jsonObject.uniqueNumber });
        }
        catch (err) {
    ...
    async updateUserUsingUAM(requestData) {
        try {
            let oldUAMData;
            let roleInstance, reportingToInstance;
            let uamInstance = {};
            let changedData = {};
            let appUserInstance;
            const xmlToJsonObj = xml_js_1.default.xml2js(requestData, { compact: true, sanitize: true });
            ...
            if (jsonObject.dateOfBirth) {
                if (!(0, moment_timezone_1.default)(jsonObject.dateOfBirth, 'DDMMYYYY').isValid()) {
                    return this.xmlResponse({ message: 'INVALID DATE OF BIRTH', statusCode: 7, uniqueNumber: jsonObject.uniqueNumber });
                }
            }
            if (jsonObject.contactNumber) {
                if (!common_1.ValidationUtils.validateMobileNumber(jsonObject.contactNumber)) {
                    return this.xmlResponse({ message: 'INVALID CONTACT NUMBER', statusCode: 7, uniqueNumber: jsonObject.uniqueNumber });
                }
            }
            ...
            const currentRole = await this.appUserRoleMappingRepository
                .findOne({
                where: {
                    appUserId: UAMData === null || UAMData === void 0 ? void 0 : UAMData.appUserId
                }
            });
            if ((currentRole === null || currentRole === void 0 ? void 0 : currentRole.appRoleId) == (foundRole === null || foundRole === void 0 ? void 0 : foundRole.id)) {
            ...
            let relatedData;
            if (appUserInstance.operationDetails.userType == common_1.Option.GLOBALOPTIONS.USERTYPE['general'].value) {
                relatedData = appUserInstance.operationDetails;
            }
            if (jsonObject.dateOfBirth &&
                relatedData &&
                (0, moment_timezone_1.default)(jsonObject.dateOfBirth, 'DDMMYYYY').format('YYYY-MM-DD') != (0, moment_timezone_1.default)(relatedData.birthDate).format('YYYY-MM-DD')) {
                stateChanged = true;
                changedData.dob = (0, moment_timezone_1.default)(jsonObject.dateOfBirth, 'DDMMYYYY').format('YYYY-MM-DD');
                uamInstance.dob = (0, moment_timezone_1.default)(jsonObject.dateOfBirth, 'DDMMYYYY');
            ...
            common_1.LoggingUtils.info(UamLogs, 'User modified');
            return this.xmlResponse({ message: 'success', statusCode: 0, uniqueNumber: jsonObject.uniqueNumber });
        }
        catch (err) {
            return this.xmlResponse({ message: 'Failure', statusCode: 1, err });
    ...
    async fetchUserIdPopulationReport(searchFilter = {}, offset = 0, limit = MAX_DATA_FETCH_LIMIT, orderBy = []) {
        ...
        return new Promise((resolve, reject) => {
            ...
            let orderByArr = [];
            underscore_1.default.each(orderBy, o => {
            ...
            return this.uamIntegrationRepository
                ...
                let query = {
                    where: {
                        id: { inq: underscore_1.default.pluck(activities, 'id') }
                    },
                    include: [
                        {
                            relation: 'appUser',
                ...
                .then((activityData) => {
                underscore_1.default.each(activityData, activity => {
                    ...
                    if (activity.appUser &&
                        ...
                        if (activity.appUser.userType == common_1.Option.GLOBALOPTIONS.USERTYPE['general'].value) {
    ...
    async exportUserIdPopulationReport(exportFormat, res, fromDate, toDate, searchFilter = {}, 
    // limit: number,
    // offset: number = 0,
    orderBy = [{ lastModifiedDate: 'DESC' }]) {
        return new Promise((resolve, reject) => {
            if (exportFormat !== 'xlsx') {
                return reject(new common_1.RestError(400, `export format should be xlsx!`, { systemcode: 1334 }));
            }
            let headers = [
            ...
            this.fetchUserIdPopulationReport(searchFilter, 0, -1, orderBy)
                ...
                let reportData = [];
                underscore_1.default.each(activityData.data, function (userIdPopulation, index) {
                    reportData.push({
                        ...
                        makerLastModifiedDateTime: (0, moment_timezone_1.default)(userIdPopulation.lastModifiedMakerDateTime, 'YYYY-MM-DD HH:mm:ss').format('DD-MMM-YYYY HH:mm:ss'),
                        checkerLastModifiedDateTime: null,
                        expiryDate: userIdPopulation.expiryDate
                            ? (0, moment_timezone_1.default)(userIdPopulation.expiryDate, 'YYYY-MM-DD HH:mm:ss').format('DD-MMM-YYYY HH:mm:ss')
                            : null,
                        creationDateTime: userIdPopulation.creationDateTime
                            ? (0, moment_timezone_1.default)(userIdPopulation.creationDateTime, 'YYYY-MM-DD HH:mm:ss').format('DD-MMM-YYYY HH:mm:ss')
                            : null,
                        dormantDate: userIdPopulation.dormantDateTime
                            ? (0, moment_timezone_1.default)(userIdPopulation.dormantDateTime, 'YYYY-MM-DD HH:mm:ss').format('DD-MMM-YYYY HH:mm:ss')
                            : null,
                        disableDateTime: userIdPopulation.disableDateTime
                            ? (0, moment_timezone_1.default)(userIdPopulation.disableDateTime, 'YYYY-MM-DD HH:mm:ss').format('DD-MMM-YYYY HH:mm:ss')
                            : null,
                        deletionDateTime: userIdPopulation.deletionDateTime
                            ? (0, moment_timezone_1.default)(userIdPopulation.deletionDateTime, 'YYYY-MM-DD HH:mm:ss').format('DD-MMM-YYYY HH:mm:ss')
                            : null,
                        userLastLoginDate: userIdPopulation.lastLoginDate
                            ? (0, moment_timezone_1.default)(userIdPopulation.lastLoginDate, 'YYYY-MM-DD HH:mm:ss').format('DD-MMM-YYYY HH:mm:ss')
                            : null
                    });
                });
                res.append('fileName', 'UserIdPopulation.xlsx');
                let excelSheet = common_1.ExcelUtils.createExcel(null, 'UserIdPopulation', headers, reportData, null);
    ...
    async updateActiveUserDetails(appUserId, details) {
        return new Promise((resolve, reject) => {
            let userType, updatedUserType, appUserRoleMappingData, updatedRoleId;
            updatedRoleId =
                details.changedData && details.changedData.appRole
                    ...
                    : details.changedData.appRoleId
                        ...
                        : details.appRole.id;
            if (!updatedRoleId) {
                return reject(new common_1.RestError(400, `App Role ID Required!`, { systemcode: 1335 }));
            }
            this.appUserRoleMappingRepository
                ...
                if (!mapping) {
                    return Promise.reject(new common_1.RestError(400, `Role not found!`, { systemcode: 1336 }));
                }
                userType = (_b = (_a = appUserRoleMappingData === null || appUserRoleMappingData === void 0 ? void 0 : appUserRoleMappingData.appUser) === null || _a === void 0 ? void 0 : _a.operationDetails) === null || _b === void 0 ? void 0 : _b.userType;
                ...
                if (updatedRoleId != appUserRoleMappingData.appRole.id) {
                    ...
                    if (!appRoleMappingInstance) {
                        return Promise.reject(new common_1.RestError(400, `App Role Mapping does not Exist!`, { systemcode: 1337 }));
                    }
                ...
                .then(() => {
                let promises = [];
                if (updatedUserType == common_1.Option.GLOBALOPTIONS.USERTYPE['general'].value) {
                    if (userType == common_1.Option.GLOBALOPTIONS.USERTYPE['general'].value) {
    ...
    saveExistingNonRMUserDetails(isExisted, appUserId, details, appRoleId) {
        return new Promise((resolve, reject) => {
            if (!details) {
                return reject(new common_1.RestError(400, `Details is required!`, { systemcode: 1329 }));
            }
            if (!details.userCode) {
                return reject(new common_1.RestError(400, `User Code is Required!`, { systemcode: 1317 }));
            }
            let innerPromise;
            if (isExisted) {
                let mapping;
                innerPromise = this.appUserRoleMappingRepository
                    ...
                    .then(async (data) => {
                    if (!data) {
                        return Promise.reject(new common_1.RestError(400, `Mapping not found!`, { systemcode: 1338 }));
                    }
                    mapping = data;
    ...
    async changeAppUserStatus(details, uniqueNumber, userProfile) {
        try {
            ...
            if (!(details && details.userCode && details.appUserStatusValue)) {
                ...
                return await Promise.reject(new common_1.RestError(400, 'Please provide valid info!', { systemcode: 1020 }));
            }
            //get the maker id
            if (!userProfile.appUserId) {
                return await Promise.reject(new common_1.RestError(465, `Couldn't identify maker`, { systemcode: 1339 }));
            }
            ...
            let appUserInstance = {}, activity = '', isLatest = true, isActive = true, statusLabel = '', disableDateTime = null, deletionDateTime = null, roleInstance = {}, uamInstance = {}, oldUAMData = {}, changedData = {};
            let rawResult, foundRole;
            if (details.appRoleId) {
                ...
                if (rawResult) {
                    ...
                    if (!rawResult.length) {
                        ...
                        return await Promise.reject(new common_1.RestError(400, 'ROLE ID DOES NOT EXITS!', { systemcode: 1340 }));
                    }
                    foundRole = await this.appRoleRepository.findOne({
                        where: {
                            id: rawResult[0]['id'],
                ...
                if (!foundRole) {
                    ...
                    return await Promise.reject(new common_1.RestError(400, 'INVALID ROLE ID!', { systemcode: 1341 }));
                }
            }
            roleInstance = foundRole;
            ...
            if (!appUser) {
                ...
                return await Promise.reject(new common_1.RestError(400, 'USERID DOES NOT EXISTS!', { systemcode: 1342 }));
            }
            appUserInstance = appUser;
            if (details.appUserStatusValue === common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.disabled.value) {
                //disable
                if (appUserInstance.appUserStatus === common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.disabled.value) {
                    await this.createUamLog({ message: 'USERID ALREADY DISABLED!', uniqueNumber: uniqueNumber });
                    return await Promise.reject(new common_1.RestError(400, 'USERID ALREADY DISABLED!', { systemcode: 1343 }));
                }
                if (appUserInstance.appUserStatus === common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.deleted.value) {
                    await this.createUamLog({ message: 'USERID IS DELETED CANNOT BE MODIFIED!', uniqueNumber: uniqueNumber });
                    return await Promise.reject(new common_1.RestError(400, 'USERID IS DELETED CANNOT BE MODIFIED!', { systemcode: 1344 }));
                }
                if (!(appUserInstance.appUserStatus === common_1.Option.GLOBALOPTIONS.APPUSERSTATUS['dormant'].value ||
                    appUserInstance.appUserStatus === common_1.Option.GLOBALOPTIONS.APPUSERSTATUS['active'].value ||
                    appUserInstance.appUserStatus === common_1.Option.GLOBALOPTIONS.APPUSERSTATUS['locked'].value)) {
                    return await Promise.reject(new common_1.RestError(400, 'USERID CAN ONLY BE EITHER ENABLED OR DELETED!', { systemcode: 1345 }));
                }
                appUserInstance.appUserStatus = common_1.Option.GLOBALOPTIONS.APPUSERSTATUS['disabled'].value;
                appUserInstance.isActive = false;
                activity = 'DISABLE';
                isLatest = true;
                isActive = false;
                statusLabel = common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.disabled.label;
                disableDateTime = new Date();
                await this.appAccessTokenRepository.updateAll({ isActive: false }, { isActive: true, appUserId: appUser.id });
            }
            if (details.appUserStatusValue === common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.deleted.value) {
                //delete
                if (appUserInstance.appUserStatus === common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.deleted.value) {
                    await this.createUamLog({ message: 'USERID ALREADY DELETED!', uniqueNumber: uniqueNumber });
                    return await Promise.reject(new common_1.RestError(400, 'USERID ALREADY DELETED!', { systemcode: 1346 }));
                }
                if (!(appUserInstance.appUserStatus === common_1.Option.GLOBALOPTIONS.APPUSERSTATUS['dormant'].value ||
                    appUserInstance.appUserStatus === common_1.Option.GLOBALOPTIONS.APPUSERSTATUS['locked'].value ||
                    appUserInstance.appUserStatus === common_1.Option.GLOBALOPTIONS.APPUSERSTATUS['disabled'].value ||
                    appUserInstance.appUserStatus === common_1.Option.GLOBALOPTIONS.APPUSERSTATUS['active'].value)) {
                    return await Promise.reject(new common_1.RestError(400, "USERID CAN'T BE DELETED!", { systemcode: 1347 }));
                }
                appUserInstance.appUserStatus = common_1.Option.GLOBALOPTIONS.APPUSERSTATUS['deleted'].value;
                appUserInstance.isActive = false;
                deletionDateTime = new Date();
                activity = 'DELETE';
                statusLabel = common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.deleted.label;
                isLatest = true;
                isActive = false;
                await this.appAccessTokenRepository.updateAll({ isActive: false }, { isActive: true, appUserId: appUser.id });
            }
            if (details.appUserStatusValue === common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.locked.value) {
                //lock
                if (appUserInstance.appUserStatus === common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.deleted.value) {
                    await this.createUamLog({ message: 'USERID ALREADY DELETED!', uniqueNumber: uniqueNumber });
                    return await Promise.reject(new common_1.RestError(400, 'USERID ALREADY DELETED!', { systemcode: 1346 }));
                ...
                if (appUserInstance.appUserStatus === common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.locked.value) {
                    ...
                    return await Promise.reject(new common_1.RestError(400, 'USERID ALREADY LOCKED!', { systemcode: 1348 }));
                }
                appUserInstance.appUserStatus = common_1.Option.GLOBALOPTIONS.APPUSERSTATUS['locked'].value;
                appUserInstance.isActive = false;
                activity = 'LOCK';
                statusLabel = common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.locked.label;
                isLatest = true;
                isActive = false;
            }
            if (details.appUserStatusValue === common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.active.value) {
                //update
                ...
                if (!UAMData) {
                    ...
                    return await Promise.reject(new common_1.RestError(400, 'UAM USERID DOES NOT EXISTS!', { systemcode: 1349 }));
                }
                uamInstance = UAMData ? UAMData.toJSON() : {};
                uamInstance.lastModifiedMakerId = makerId.userCode;
                uamInstance.lastModifiedCheckerId = makerId.userCode;
                oldUAMData = UAMData;
                if (UAMData.status === common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.locked.label)
                    activity = 'UNLOCK';
                else if (UAMData.status === common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.disabled.label)
                    activity = 'ENABLE';
                else if (UAMData.status === common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.deleted.label)
                    activity = 'REOPEN';
                else if (UAMData.status === common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.dormant.label) {
                    activity = 'ENABLE';
                ...
                if (details.dob &&
                    relatedData &&
                    (0, moment_timezone_1.default)(details.dob, 'DDMMYYYY').format('YYYY-MM-DD') != (0, moment_timezone_1.default)(relatedData.birthDate).format('YYYY-MM-DD')) {
                    changedData.dob = (0, moment_timezone_1.default)(details.dob, 'DDMMYYYY').format('YYYY-MM-DD');
                    uamInstance.dob = (0, moment_timezone_1.default)(details.dob, 'DDMMYYYY').add(5, 'hours').add(30, 'minutes');
                    uamInstance.newDob = uamInstance.dob;
                    uamInstance.oldDob = (0, moment_timezone_1.default)(relatedData.birthDate).add(5, 'hours').add(30, 'minutes');
                }
                else {
                    uamInstance.oldDob = null;
                    uamInstance.newDob = null;
                    if (details.dob === "") {
                        ...
                        uamInstance.dob = relatedData ? (0, moment_timezone_1.default)(relatedData.birthDate).add(5, 'hours').add(30, 'minutes') : uamInstance.dob.add(5, 'hours').add(30, 'minutes');
                    }
                    appUserInstance.dob = relatedData ? relatedData.birthDate : appUserInstance.dob;
                }
                ...
                uamInstance.isActive = true;
                statusLabel = common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.active.label;
                ...
                common_1.LoggingUtils.info(UamLogs, 'User updated');
            }
            else {
                appUserInstance.previousStatus = appUserInstance.appUserStatus;
                appUserInstance.lastModifiedDate = new Date();
                ...
                if (uamLog) {
                    ...
                    UAMData.newProfileName = null;
                    if (userEnabledFromDormancy)
                        UAMData.enabledDormancyRecently = true; // if the user is being enabled from dormancy we set this flag
                    await this.uamIntegrationRepository.updateAll(uamLog, { id: uamLog.id });
                    const updateUamIntegration = await this.create(UAMData);
                    if (!updateUamIntegration) {
                        throw new Error('Error while updating UAM Integration');
            ...
            return Promise.reject(new common_1.RestError(400, 'Something went wrong', { systemcode: 1244 }));
        }
    }
    async createUser(details, uniqueNumber, userProfile) {
        try {
            let appRoleInstance, appUserInstance = {};
            let emailParts = '';
            if (details.email) {
                emailParts = details.email.split('@');
            }
            if (!details.email ||
                !common_1.ValidationUtils.validateEmail(details.email) ||
                !(emailParts[1] && ['HDFCBANK.COM', 'IN.HDFCBANK.COM'].includes(emailParts[1].toUpperCase()))) {
                await this.createUamLog({ message: 'INVALID EMAILID!', uniqueNumber: uniqueNumber });
                return await Promise.reject(new common_1.RestError(400, 'INVALID EMAILID!', { systemcode: 1350 }));
            }
            if (!details.userCode) {
                await this.createUamLog({ message: 'INVALID USER CODE!', uniqueNumber: uniqueNumber });
                return await Promise.reject(new common_1.RestError(400, 'INVALID USER CODE!', { systemcode: 1351 }));
            }
            if (!details.employeeCode || !/^[a-zA-Z0-9]{1,20}$/i.test(details.employeeCode)) {
                ...
                return await Promise.reject(new common_1.RestError(400, 'INVALID EMPLOYEE CODE!', { systemcode: 1331 }));
            }
            //@todo - maybe look at this later but for now set the user type to 2
            details['userType'] = 2;
            if (!details.userType || [common_1.Option.GLOBALOPTIONS.USERTYPE['general'].value].indexOf(Number(details.userType)) == -1) {
                await this.createUamLog({ message: 'INVALID USER TYPE!!', uniqueNumber: uniqueNumber });
                return await Promise.reject(new common_1.RestError(400, 'INVALID USER TYPE!', { systemcode: 1352 }));
            }
            if (!details.category || categories.indexOf(Number(details.category)) == -1) {
                ...
                return await Promise.reject(new common_1.RestError(400, 'INVALID CATEGORY!', { systemcode: 1353 }));
            }
            if (!details.name || !/^[a-zA-Z0-9' ]*$/.test(details.name)) {
                await this.createUamLog({ message: 'INVALID USER NAME!', uniqueNumber: uniqueNumber });
                return await Promise.reject(new common_1.RestError(400, 'INVALID USER NAME!', { systemcode: 1354 }));
            }
            if (details.dob) {
                if (!(0, moment_timezone_1.default)(details.dob, 'DDMMYYYY').isValid()) {
                    await this.createUamLog({ message: 'INVALID DATE OF BIRTH!', uniqueNumber: uniqueNumber });
                    return await Promise.reject(new common_1.RestError(400, 'INVALID DATE OF BIRTH!', { systemcode: 1332 }));
                }
            }
            if (details.contactNumber) {
                if (!common_1.ValidationUtils.validateMobileNumber(details.contactNumber)) {
                    await this.createUamLog({ message: 'INVALID CONTACT NUMBER!', uniqueNumber: uniqueNumber });
                    return await Promise.reject(new common_1.RestError(400, 'INVALID CONTACT NUMBER!', { systemcode: 1355 }));
                }
            }
            else {
            ...
            if (details.salutation) {
                if (salutations.indexOf(Number(details.salutation)) == -1) {
                    ...
                    return await Promise.reject(new common_1.RestError(400, 'INVALID SALUTATION!', { systemcode: 1356 }));
                }
            }
            if (details.gender) {
                if (genders.indexOf(Number(details.gender)) == -1) {
                    await this.createUamLog({ message: 'INVALID GENDER!', uniqueNumber: uniqueNumber });
                    return await Promise.reject(new common_1.RestError(400, 'INVALID GENDER!', { systemcode: 1357 }));
                }
            }
            if (!details.branchCode || details.branchCode.length === 0) {
                ...
                return await Promise.reject(new common_1.RestError(400, 'INVALID BRANCH CODE!', { systemcode: 1358 }));
            }
            if (!details.branchName || details.branchName.length === 0) {
                ...
                return await Promise.reject(new common_1.RestError(400, 'INVALID BRANCH NAME!', { systemcode: 1359 }));
            }
            if (!details.departmentCode || details.departmentCode.length === 0) {
                ...
                return await Promise.reject(new common_1.RestError(400, 'INVALID DEPARTMENT CODE!!', { systemcode: 1360 }));
            }
            if (!details.departmentName || details.departmentCode.length === 0) {
                ...
                return await Promise.reject(new common_1.RestError(400, 'INVALID DEPARTMENT NAME!!', { systemcode: 1361 }));
            }
            ...
            if (!foundRole) {
                ...
                return await Promise.reject(new common_1.RestError(400, 'INVALID ROLE!', { systemcode: 1362 }));
            }
            appRoleInstance = foundRole;
            const rawAppUser = await this.appUserRepository.find({
                where: {
            ...
            if (rawAppUser &&
                ...
                return await Promise.reject(new common_1.RestError(400, 'USERID/EMPCODE ALREADY EXISTS!', { systemcode: 1363 }));
            }
            const maxPossibleLoginAttemots = await this.uamLoginAttemptsConfigRepository.findById(1);
            let dobToSave = details.dob ? moment_timezone_1.default.utc(details.dob, 'DDMMYYYY').format('YYYY-MM-DD') : null;
            const savedUser = await this.saveNonRMUserDetails({
                currentAppUser: { id: null }
            }, {
                name: details.name,
    ...
    async downloadAdminActivityReport(res, filter, options) {
        let headers = [
        ...
        try {
            ...
            for (let activityReport of data) {
                if (activityReport.activity === 'LOCK' || activityReport.activity == 'AUTOMATED DORMANCY')
                    continue; // we want to skip records where the activity is lock
                formatData.push({
                    ...activityReport,
                    lastModifiedMakerDateTime: (0, moment_timezone_1.default)(activityReport.lastModifiedMakerDateTime, 'YYYY-MM-DD HH:mm:ss').format('DD-MMM-YYYY HH:mm:ss'),
                    lastModifiedCheckerDateTime: null
                });
            }
            let excelSheet = common_1.ExcelUtils.createExcel(null, 'Admin Activity Report', headers, formatData, null);
            const result = await excelSheet.xlsx.writeBuffer();
            return result;
        }
        catch (error) {
            common_1.LoggingUtils.error('Some Error Occured');
            return new common_1.RestError(400, 'Error occured while exporting Admin Activity Report', { systemcode: 1364 });
        }
    }
    async updateMaxAllowedLoginAttempts(configs) {
        try {
            if (!configs.maxDormancyDays && !configs.maxLoginAttempts && !configs.maxDormancyDaysBeforeFirstLogin)
                return Promise.reject(new common_1.RestError(422, 'Please provide atleast one config parameter', { systemcode: 1365 }));
    ...
    getAllParamsFromRequest(jsonObject, operation) {
        // try {
    ...
    removeEmptyParams(jsonObject) {
        if (jsonObject.emailId && jsonObject.emailId.length > 0)
            delete jsonObject.emailId;
        if (jsonObject.userName && jsonObject.userName.length > 0)
            delete jsonObject.userName;
        if (jsonObject.uniqueNumber && jsonObject.uniqueNumber.length > 0)
            delete jsonObject.uniqueNumber;
        if (jsonObject.contactNumber && jsonObject.contactNumber.length > 0)
            delete jsonObject.contactNumber;
        if (jsonObject.roleId && jsonObject.roleId.length > 0)
            delete jsonObject.roleId;
        if (jsonObject.empCode && jsonObject.empCode.length > 0)
            delete jsonObject.empCode;
        if (jsonObject.employeeCode && jsonObject.employeeCode.length > 0)
            delete jsonObject.employeeCode;
        if (jsonObject.userType && jsonObject.userType.length > 0)
            delete jsonObject.userType;
        if (jsonObject.category && jsonObject.category.length > 0)
            delete jsonObject.category;
        if (jsonObject.dateOfBirth && jsonObject.dateOfBirth.length > 0)
            delete jsonObject.dateOfBirth;
        if (jsonObject.salutation && jsonObject.salutation.length > 0)
            delete jsonObject.salutation;
        if (jsonObject.gender && jsonObject.gender.length > 0)
            delete jsonObject.gender;
        if (jsonObject.branchCode && jsonObject.branchCode.length > 0)
            delete jsonObject.branchCode;
        if (jsonObject.branchName && jsonObject.branchName.length > 0)
            delete jsonObject.branchName;
        if (jsonObject.departmentCode && jsonObject.departmentCode.length > 0)
            delete jsonObject.departmentCode;
        if (jsonObject.departmentName && jsonObject.departmentName.length > 0)
            delete jsonObject.departmentName;
    }
    ...
    async returnAppUserStatusLabel(appUserStatus) {
        if (appUserStatus === common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.disabled.value)
            return common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.disabled.label;
        if (appUserStatus === common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.active.value)
            return common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.active.label;
        if (appUserStatus === common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.deleted.value)
            return common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.deleted.label;
        if (appUserStatus === common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.dormant.value)
            return common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.dormant.label;
        if (appUserStatus === common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.locked.value)
            return common_1.Option.GLOBALOPTIONS.APPUSERSTATUS.locked.label;
    }
    async downloadRoleRightsReport(res, filter, options) {
        let headers = [
        ...
        try {
            ...
            let excelSheet = common_1.ExcelUtils.createExcel(null, 'Role Rights Report', headers, newData, null);
            const result = await excelSheet.xlsx.writeBuffer();
            return result;
        }
        catch (error) {
            common_1.LoggingUtils.error('Some Error Occured');
            return new common_1.RestError(400, 'Error occured while exporting Role Rights Report');
        }
    }
    async addDepartmentToResults(data) {
        if (data && data.length == 0)
            return data; // return if data is empty
        //find out all unique combinations of profile and departmentName
        let departmentsForAllroles = await this.uamIntegrationRepository.execute("select upper(department_name) as department, upper(profile) as profile from uam_integration ui where department_name is not null and profile is not null and department_name <> '' and profile <> '' and is_latest = true and status <> 'Deleted' group by upper(department_name), upper(profile)");
        if (underscore_1.default.isEmpty(departmentsForAllroles)) {
            departmentsForAllroles = [];
        }
        const profileWiseDepartments = {}; // This will hold the profile/role as key and a concatenation of all departments as value
        //here we populate profileWiseDepartments
        departmentsForAllroles.map((record) => {
...
UamIntegrationFacade = (0, tslib_1.__decorate)([
    (0, core_1.injectable)({ scope: core_1.BindingScope.APPLICATION }),
    (0, tslib_1.__param)(0, (0, repository_1.repository)(common_1.UamIntegrationRepository)),
    (0, tslib_1.__param)(1, (0, repository_1.repository)(common_1.AppRoleRepository)),
    (0, tslib_1.__param)(2, (0, repository_1.repository)(common_1.AppUserRepository)),
    (0, tslib_1.__param)(3, (0, repository_1.repository)(common_1.UAMLogRepository)),
    (0, tslib_1.__param)(4, (0, repository_1.repository)(common_1.AppUserRoleMappingRepository)),
    (0, tslib_1.__param)(5, (0, repository_1.repository)(common_1.OperationRepository)),
    (0, tslib_1.__param)(6, (0, repository_1.repository)(common_1.UamLoginAttemptsConfigRepository)),
    (0, tslib_1.__param)(7, (0, repository_1.repository)(common_1.AppAccessTokenRepository)),
    (0, tslib_1.__param)(8, (0, repository_1.repository)(common_1.RoleRightsRepository)),
    (0, tslib_1.__metadata)("design:paramtypes", [common_1.UamIntegrationRepository,
        common_1.AppRoleRepository,
        common_1.AppUserRepository,
        common_1.UAMLogRepository,
        common_1.AppUserRoleMappingRepository,
        common_1.OperationRepository,
        common_1.UamLoginAttemptsConfigRepository,
        common_1.AppAccessTokenRepository,
        common_1.RoleRightsRepository])
], UamIntegrationFacade);
exports.UamIntegrationFacade = UamIntegrationFacade;
//# sourceMappingURL=uam-integration.facade.js.map